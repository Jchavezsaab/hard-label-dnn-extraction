...
def saveDuals(duals, name):
    os.makedirs(f"../data/duals/{name}", exist_ok=True)
    for layer in range(len(duals)):
        for neuron in range(len(duals[layer])):
            np.save(f"../data/duals/{name}/{layer+1}_{neuron}.npy", np.array(duals[layer][neuron]))
...
def main():
    ...
    model = tf.keras.models.load_model(f"../data/{name}.h5")
    Nlayers = 0
    weights, biases = [],[]
    ...
    while True:
        ...
        if N % 1000 == 0:
            ...
            print(f"Saved dual points to ../data/duals/{name}/")
