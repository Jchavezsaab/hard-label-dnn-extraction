...
def leakydnn(input_shape, hidden_sizes, num_classes, leaky=False):
    """
    ...
    for layer in model.layers:
        if type(layer) != layers.Dense:
            continue
        dim, n = layer.get_weights()[0].shape
        # weights = np.array([randomUnitVector(dim) for i in range(n)]).T
        b = []
        for _ in range(dim):
...
model = leakydnn([32], [8]*4, 4, leaky=False)
model.save("unitary_32_8x4_4_float64.keras")