...
NEURONIDS                = range(3) # neuron IDs to analyze
...
nThreads                  = 3 # Number of threads to be used for analyzing multiple neurons in parallel
# ==================================== #
...
if __name__ == '__main__':
    ...
    for layerID in LAYERIDS:
        ...
        for neuronID in NEURONIDS:
            ...
            if nThreads > 1: pool.apply_async(func=sign_recovery_whitebox.main, args=[args.split(' ')], error_callback=error_handler)
            else: 
                args = args.split(' ')
                sign_recovery_whitebox.main(args)
    if nThreads > 1: 
        pool.close()
        pool.join()
