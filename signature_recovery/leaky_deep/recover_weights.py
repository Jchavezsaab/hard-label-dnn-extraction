...
class CIFAR10NetPrefix(nn.Module):
    def __init__(self, layers):
        ...
        if layers == 0:
            ...
            dims = [IDIM] + [DIM] * (NUM_LAYERS - 2) + [SHRINK]
            self.fcs = nn.Sequential(*[nn.Linear(dims[i], dims[i+1]) for i in range(min(layers, NUM_LAYERS - 1))])
        self.double()

    def relu_around(self, x):
        mask = (x[:1]>=0).to(torch.float64)
        return x * mask + (x*.01) * (1-mask)
...
def transfer_weights(source_model, target_model, source_prefix='', target_prefix='fcs'):
    ...
    layer_count = 0
    while True:
        source_weight_key = f'{source_prefix}fcs.{layer_count}.weight'
        source_bias_key = f'{source_prefix}fcs.{layer_count}.bias'
...
def verify_plane_invariance(left, nleft):
    """Verify that small perturbations along the plane maintain zero output"""
    # Verify that cheat_net_cpu.cheat(left)[1][:,1] is basically zero
    #print(gap(torch.tensor(left)))
    
    # Create a small perturbation orthogonal to nleft (on the plane)
    # Generate random vector and project out the normal component
    random_vec = np.random.randn(len(left))
    random_vec = random_vec - np.dot(random_vec, nleft) * nleft
    random_vec = random_vec / np.linalg.norm(random_vec)  # normalize
    
    # Test small step along the plane
    epsilon = 1e-4
    perturbed = left + random_vec * epsilon
    #print(gap(torch.tensor(perturbed)))
    #print()

def is_consistent_help(points, prefix, layer=0, do_return_soln=False, allow_close=False, deltacheck=False):
    samples = []
    ...
    for i, (left, x0, right) in enumerate(points):
        ...
        # Verify plane invariance
        verify_plane_invariance(left, nleft)
        verify_plane_invariance(right, nright)

        y0, N = intersect(left, right, nleft, nright)
        ...
        #print('d1', cheat_net_cpu.cheat(torch.tensor(np.array([x0])))[1][:,1])

        #moved = np.array(points)# - cheat_net_cpu.fcs[0].bias.detach().cpu().numpy()
        #print('d1', moved @ cheat_net_cpu.fcs[0].weight.detach().cpu().numpy()[11,:])

        #print('dfind', moved @ np.load("/tmp/a.npy"))
        #print('dfind2', moved @ np.load("/tmp/b.npy"))

        points = np.concatenate(([x0], points), 0)

        
        #print(points)
        #print('d1', cheat_net_cpu.cheat(torch.tensor([x0]))[1])
        #print('d1', cheat_net_cpu.fcs[1](torch.nn.functional.leaky_relu(cheat_net_cpu.fcs[0](torch.tensor([x0])))))

        #print('d1', torch.tensor([x0]) @ cheat_net_cpu.fcs[0].weight.detach().cpu().numpy()[:,1] + cheat_net_cpu.fcs[0].bias[1])
        
        points = prefix.forward_around(torch.tensor(points).cuda()).cpu()

        #print('dx', cheat_net_cpu.fcs[1]((points[:1])))

        #print('d2', torch.tensor([x0]))
        #print('sdv1', torch.linalg.svdvals(points - torch.mean(points, dim=0)).cpu().numpy())
        #exit(0)
    ...
    if deltacheck:
        return np.log(S[len(S)-all_zero-1])/np.log(10) - np.log(S[len(S)-all_zero-2])/np.log(10)
    return S[len(S)-all_zero-1]

def is_consistent(points, prefix, layer=0, do_return_soln=False, deltacheck=False):
    try:
        return is_consistent_help(points, prefix, layer, do_return_soln, deltacheck=deltacheck)
    except MathIsHard:
        return None
