...
class CIFAR10Net(nn.Module):
    def __init__(self):
        ...
        # First layer
        self.fcs.append(nn.Linear(IDIM, DIM))
        
        # Middle layers (all DIM to DIM)
        for i in range(NUM_LAYERS - 3):
            self.fcs.append(nn.Linear(DIM, DIM))
        
        # Second to last layer (DIM to SHRINK)
        self.fcs.append(nn.Linear(DIM, SHRINK))
        
        # Last layer (SHRINK to 10)
        self.fcs.append(nn.Linear(SHRINK, 10))
...
def load_model_with_remapped_keys(model, path):
    state_dict = torch.load(path)
    new_state_dict = {}
    for key, value in state_dict.items():
        if key.startswith('fc') and not key.startswith('fcs'):
            layer_num = int(key[2:].split('.')[0]) - 1
            new_key = f'fcs.{layer_num}.' + key.split('.', 1)[1]
...
if not TINY:
    load_model_with_remapped_keys(cheat_net_cuda, "models/final.pth")
else:
    load_model_with_remapped_keys(cheat_net_cuda, "models/deep.pth")
cheat_net_cuda.double().cuda()

cheat_net_cpu = CIFAR10Net()
if not TINY:
    load_model_with_remapped_keys(cheat_net_cpu, "models/final.pth")
else:
    load_model_with_remapped_keys(cheat_net_cpu, "models/deep.pth")
cheat_net_cpu.double()

cheat_solution = [x.cpu().detach().numpy() for x in cheat_net_cpu.fcs.parameters() if len(x.shape) == 2]
