...
def is_consistent_help(points, prefix, layer=0, do_return_soln=False, allow_close=False):
    ...
    for i, (left, x0, right) in enumerate(points):
        ...
        #moved = np.array(points)# - cheat_net_cpu.fc1.bias.detach().cpu().numpy()
        #print('d1', moved @ cheat_net_cpu.fc1.weight.detach().cpu().numpy()[11,:])
        ...
        print('d1', cheat_net_cpu.fc2(torch.nn.functional.leaky_relu(cheat_net_cpu.fc1(torch.tensor([x0])))))

        #print('d1', torch.tensor([x0]) @ cheat_net_cpu.fc1.weight.detach().cpu().numpy()[:,1] + cheat_net_cpu.fc1.bias[1])
        
        points = prefix.forward_around(torch.tensor(points).cuda()).cpu()

        print('dx', cheat_net_cpu.fc2((points[:1])))
